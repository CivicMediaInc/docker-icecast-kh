...
<xsl:variable name = "theIP" ><xsl:value-of select="IP" /></xsl:variable>
		<tr>
                <td align="center"><a href="https://www.nic.ru/whois/?query={$theIP}" target="_blank"><xsl:value-of select="IP" /></a><xsl:if test="username"> (<xsl:value-of select="username" />)</xsl:if></td>
<!-- /modified -->
